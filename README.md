...
## 2. Host de alojamiento de elementos virtuales
...
## 3. Elementos físicos a virtualizar
...
10. Captura de la salida stdout y stderr que produce la ejecución en linux y almacenamiento del resultado en Windows junto con los scripts ejecutados, con denominación **milog\<nnn>.log.** La salida de los scripts puede mostrarse en una ventana mientras se ejecutan o solamente almacenarse en disco.
   ...
   > La relación entre el proceso PowerShell y las máquinas virtuales se establece mediante invocaciones a las funcionalidades del componente de virtualBox VBoxManage
...
## 5. Tareas previas a la instalación
...
Antes de proceder a la ejecución de los procesos de instalación automatizada hay que configurar y obtener ciertos elementos y realizar algunas acciones en el Host:
   ...
   2. Configuración de Firewall. Es probable que ciertas peticiones de echo ICMP no se ejecuten correctamente entre el host y las máquinas virtuales. Para solucionarlo es necesario habilitar una regla de entrada (la regla probablemente exista pero suele estar deshabilitada). En primer lugar verificaremos si la regla existe y si está habilitada. Puede hacerse desde línea de comandos PowerShell como administrador:
 
      ```
      Get-NetFirewallRule -name "vm-monitoring-icmpv4"
      ```
   
      2.1. Si la regla existe y el atributo “Enabled” presenta valor “True”. En ese caso la regla ya estará activada y no hay que hacer nada mas.

      2.2. Si la regla existe y el atributo “Enabled” presenta valor “False”. En ese caso habrá que habilitarla desde línea de comandos PowerShell como dministrador mediante:
      
        ```
        Set-NetFirewallRule -name "vm-monitoring-icmpv4" -Enabled True
        ```

      2.3 Si la regla no existe. En ese caso debe crearse desde línea de comandos owerShell como administrador mediante:
   ...
   3. Identificar el nombre del dispositivo de red que permite la salida a Internet y sobre el que se construirá la red pública de la solución. Típicamente Será ethernet cableada o WiFi. Se puede localizar desde PowerShell como administrador. El resultado será una expresión similar a: ` Intel(R) Dual BandWireless-AC 8265 `. Debe anotarse ese nombre del dispositivo para utilizarlo EXACTAMENTE como aparece en la salida del comando en el parámetro de configuración correspondiente.
...
```
   ...
   5. Descargar los componentes Oracle GI y DB 19.3 para Linux x86_64 en formato .zip desde la web de Oracle <https://www.oracle.com/es/database/technologies/oracle19c-linux-downloads.html>. Ambos ficheros deberán guardarse en el mismo directorio. y son:
   ...
   7. Crear el fichero de definición de infraestructura. Se debe construir con los parámetros necesarios con una estructura JSON. Por defecto se buscará el fichero “params.json” en el mismo directorio que los programas ejecutables. Se proporciona un ejemplo junto con el software que puede ser editado con los valores que correspondan . Los parámetros de dicho fichero se detallan a continuación:
...
## 6. Ejecución de los procesos de instalación del sistema

A continuación se describen los procesos ejecutables que se necesitan para crear el sistema.
...
### 6.4. Instalar Software de base de datos Oracle
...
### 6.6. Crear Base de datos DataGuard Standby
...
### 6.7. Configurar DataGuard Broker
